...
func (eb *EventBus) Fire(event Event) {
	...
	for _, handler := range handlers {
		func() {
			defer func() {
				if r := recover(); r != nil {
					log.Error("PANIC recovered in event handler", "function", "Fire", "event_type", event.Type, "error", r)
				}
			}()
			handler(event)
		}()
...
func (eb *EventBus) FireAsync(event Event) {
	...
	for subscriptionID, handler := range handlers {
		go func(id string, h EventHandler) {
			defer func() {
				if r := recover(); r != nil {
					log.Error("PANIC recovered in async event handler", "function", "FireAsync", "subscription_id", id, "event_type", event.Type, "error", r)
				}
			}()
			h(event)
